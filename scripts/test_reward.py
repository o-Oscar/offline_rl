...
from offline_rl.utils.dataset.helper import batchify, to_np, to_torch

model_save_path = Path("results/models/reward_0")
model = ConvMDN((8, 8, 6), 1)
model.load(model_save_path / "model_9")
...
# state_np = create_state(-3, -2)


# print(np.where(all_states == state_np))


for state_np in states_np:
    plt.figure()
    state_th = th.Tensor(state_np)
    states_th = state_th.view((1, 8, 8, 6))
...
for state_np in states_np:
    ...
    plt.hist(np.array(state_rewards).flatten(), range=(np.min(xs), np.max(xs)), bins=30)
